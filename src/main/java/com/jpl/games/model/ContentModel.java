...
/**
 * 3D Content Model for Viewer App. Contains the 3D subscene and everything related to it: light,
 * cameras, axis Based on com.javafx.experiments.jfx3dviewer.ContentModel
 *
...
public class ContentModel {
      ...
      event -> {
        ...
        if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
          ...
          if (event.isMiddleButtonDown()
              || (event.isPrimaryButtonDown() && event.isSecondaryButtonDown())) {
            cameraXform2.setTx(
                cameraXform2.t.getX() + xFlip * mouseDeltaX * modifierFactor * modifier * 0.3);
            cameraXform2.setTy(
                cameraXform2.t.getY() + yFlip * mouseDeltaY * modifierFactor * modifier * 0.3);
          } else if (event.isPrimaryButtonDown()) {
            cameraXform.setRy(
                cameraXform.ry.getAngle() - yFlip * mouseDeltaX * modifierFactor * modifier * 2.0);
            cameraXform.setRx(
                cameraXform.rx.getAngle() + xFlip * mouseDeltaY * modifierFactor * modifier * 2.0);
  ...
  private final EventHandler<ZoomEvent> zoomEventHandler =
      event -> {
