  ...
  */
  ...
  public int correctPositions() {
    ...
    for (int x = 0; x < 3; x++) {
      for (int y = 0; y < 3; y++) {
        for (int z = 0; z < 3; z++) {
            if(cube[x][y][z] == correctCube[x][y][z]){
                correct++;
            }
        }
      }
  ...
  public List<Integer> getCubeLinear() {
  ...
  /* returns 3D array as a flatten list of indexes */
  public List<Integer> getCorrectCubeLinear() {
    List<Integer> newArray = new ArrayList<>(27);
    for (int f = 0; f < 3; f++) {
      for (int l = 0; l < 3; l++) {
        for (int a = 0; a < 3; a++) {
          newArray.add(correctCube[f][l][a]);
        }
      }
    }
    return newArray;
  ...
  public void printCube() {
    List<Integer> newArray = getCubeLinear();
    for (int i = 0; i < 27; i++) {
      if (i == 9 || i == 18) {
        System.out.print(" ||");
      }
